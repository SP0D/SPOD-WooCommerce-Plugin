...
class ApiSubscriptions extends ApiHandler
{
    ...
    {
        $hook_url = $this->getWebhookFileUrl().'?type='.$type;
        $data = [
            'eventType' => $this->subscription_types[$type],
            'url' => $hook_url,
            'secret' => get_option('ng_spod_plugin_token')
        ];
    ...
    {
        return get_bloginfo('url').'/wp-content/plugins/spod_woocommerce_plugin/webhook.php';
    }
